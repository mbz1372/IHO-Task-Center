...
const HotelLifecycleHealthV19=dynamic(()=>import('@/components/enterprise/V19DataTruth').then(m=>m.HotelLifecycleHealthV19),{ssr:false});
...
function IHOSApp(){
  ...
  return <div className={cn("appShell",sidebarCollapsed&&"sidebarCollapsed")}>
    ...
    <main className="main">
      <header className="topbar v15Topbar"><div className="topbarPrimaryV15"><button type="button" className="navToggleTopV15" onClick={()=>setSidebarCollapsed(!sidebarCollapsed)} title={sidebarCollapsed?'باز کردن منو':'جمع کردن منو'}>{sidebarCollapsed?<PanelRightOpen/>:<PanelRightClose/>}</button><div className="search"><Search size={18}/><input placeholder="جستجو در هتل، تسک، کارشناس، برچسب..." value={q} onChange={e=>setQ(e.target.value)}/></div></div><div className="actions"><GuideHint topic={view}/><span className={cn('statusPill',online?'on':'off')}>{online?<Wifi/>:<WifiOff/>}{online?'آنلاین':'محلی'}</span>{can('tasks_create')&&<button className="btn primary" onClick={()=>openModal('task')}><Plus/> تسک جدید</button>}<button type="button" className="iconBtn themeToggleV15" onClick={toggleTheme} title={isDarkTheme?'حالت روشن':'حالت شب'}>{isDarkTheme?<Sun/>:<Moon/>}</button><button className="iconBtn notifBtn" onClick={()=>setView('notifications')}>{unread>0&&<i/>}<Bell/></button><button className="iconBtn" onClick={syncAll}><RefreshCw/></button><div className="userMini"><b>{me.full_name}</b><small>{currentRole?.title||me.role}</small></div><button className="iconBtn" onClick={()=>setMe(null)}><LogOut/></button></div></header>
