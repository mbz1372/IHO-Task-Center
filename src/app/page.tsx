...
import { activeStatuses, categories, checklistPresets, experts, managers, priorities, statuses, Task, Status, Category, User, users } from '@/lib/types';
import { hasSupabase, supabase } from '@/lib/supabase';

type Tab = 'dashboard' | 'kanban' | 'list' | 'hotels' | 'team' | 'settings';
...
export default function App(){
  ...
  function emptyForm(): FormState { return { title:'', hotelId:Number(hotels[0]?.id || 1), category:'ظرفیت', priority:'متوسط', status:'جدید', assignee:experts[0], manager:managers[0], dueDate:today(), description:'', tags:'' }; }
  ...
  return <main className="p-4 lg:p-6 max-w-[1600px] mx-auto">
    <header className="flex flex-wrap justify-between items-center gap-3 mb-5">
      ...
      <div className="flex flex-wrap gap-2"><button className="btn secondary">{syncMode === 'supabase' ? <Wifi size={18}/> : <WifiOff size={18}/>} {syncMode === 'supabase' ? 'آنلاین Supabase' : 'Local Mode'}</button><button className="btn secondary"><Bell size={18}/> {kpi.late + kpi.urgent}</button><button className="btn" onClick={()=>openCreate()}><Plus size={18}/> تسک جدید</button><button className="btn secondary" onClick={()=>setLogged(false)}><LogOut size={18}/> خروج</button></div>
    </header>
    <nav className="flex gap-2 overflow-auto mb-5 pb-1">{tabs.map(t => <button key={t.id} onClick={()=>setTab(t.id)} className={`navTab ${tab===t.id?'active':''}`}>{t.title}</button>)}</nav>
    ...
    {loading ? <div className="card p-8 text-center">در حال بارگذاری...</div> : <>
      ...
      {tab === 'team' && <TeamPage tasks={visible}/>} 
      {tab === 'settings' && <SettingsPage/>}
    </>}
    {showForm && <TaskForm form={form} setForm={setForm} save={saveForm} close={()=>setShowForm(false)}/>} 
...
const tabs: {id:Tab; title:string}[] = [
  {id:'dashboard', title:'داشبورد'}, {id:'kanban', title:'کانبان'}, {id:'list', title:'لیست تسک‌ها'}, {id:'hotels', title:'پرونده هتل‌ها'}, {id:'team', title:'تیم'}, {id:'settings', title:'تنظیمات'}
];
