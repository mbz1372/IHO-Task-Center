...
const HotelSuperApp=dynamic(()=>import('@/components/superapp/HotelSuperApp'),{ssr:false,loading:()=> <div className="moduleLoadingV16"><span/><b>در حال بارگذاری سوپر اپ هتل...</b></div>});
const OperationsDashboard=dynamic(()=>import('@/components/dashboard/OperationsDashboard'),{ssr:false,loading:()=> <div className="moduleLoadingV16"><span/><b>در حال آماده‌سازی داشبورد...</b></div>});
import { createClient } from '@supabase/supabase-js';
import { Activity, AlarmClock, Archive, ArrowLeft, Bell, Building2, CalendarDays, CheckCircle2, CheckSquare, ChevronDown, ChevronLeft, ChevronRight, ClipboardList, Clock3, Database, Download, Edit3, Eye, FileCheck2, FileText, Filter, Flag, FolderKanban, FolderOpen, Hotel, KeyRound, LayoutDashboard, ListChecks, LogOut, Menu, Moon, MoreHorizontal, Palette, PanelRightClose, PanelRightOpen, Pin, Plus, RefreshCw, Rocket, Save, Search, Settings, Shield, ShieldAlert, SlidersHorizontal, Sparkles, Square, Sun, Tag, Target, Trash2, Upload, UserPlus, Users, Wifi, WifiOff, X } from 'lucide-react';
...
// Types
type PermissionKey = 'dashboard'|'inbox'|'executive'|'crm360'|'reports'|'riskRadar'|'tasks'|'tasks_create'|'tasks_edit'|'tasks_delete'|'assignments'|'approvals'|'sla'|'playbooks'|'messages'|'hotels'|'hotels_import'|'contracts'|'communications'|'calendar'|'documents'|'documents_upload'|'team'|'roles'|'logs'|'reminders'|'automations'|'goals'|'projects'|'savedViews'|'bulkActions'|'aiAssistant'|'dailyReport'|'settings'|'notifications'|'hotelSuperApp';
...
function IHOSApp(){
  ...
  return <div className={cn("appShell",sidebarCollapsed&&"sidebarCollapsed")}>
    ...
    <main className="main">
      <header className="topbar v15Topbar"><div className="topbarPrimaryV15"><button type="button" className="navToggleTopV15" onClick={()=>setSidebarCollapsed(!sidebarCollapsed)} title={sidebarCollapsed?'باز کردن منو':'جمع کردن منو'}>{sidebarCollapsed?<PanelRightOpen/>:<PanelRightClose/>}</button><div className="search"><Search size={18}/><input placeholder="جستجو در هتل، تسک، کارشناس، برچسب..." value={q} onChange={e=>setQ(e.target.value)}/></div></div><div className="actions"><span className={cn('statusPill',online?'on':'off')}>{online?<Wifi/>:<WifiOff/>}{online?'آنلاین':'محلی'}</span>{can('tasks_create')&&<button className="btn primary" onClick={()=>openModal('task')}><Plus/> تسک جدید</button>}<button type="button" className="iconBtn themeToggleV15" onClick={toggleTheme} title={isDarkTheme?'حالت روشن':'حالت شب'}>{isDarkTheme?<Sun/>:<Moon/>}</button><button className="iconBtn notifBtn" onClick={()=>setView('notifications')}>{unread>0&&<i/>}<Bell/></button><button className="iconBtn" onClick={syncAll}><RefreshCw/></button><div className="userMini"><b>{me.full_name}</b><small>{currentRole?.title||me.role}</small></div><button className="iconBtn" onClick={()=>setMe(null)}><LogOut/></button></div></header>
      <section className="content">
        ...
        {view==='crm360'&&can('crm360')&&<CRM360 hotels={hotels} tasks={visibleTasks} docs={docs} users={users} setView={setView} exportHotels={()=>csv(hotels,'ihos-hotels.csv')} onCreateTask={(h:any)=>{setEditing({id:uid(),title:`پیگیری ${h.title}`,hotel_id:h.id,hotel_title:h.title,city:h.city,priority:'متوسط',status:settings.defaultTaskStatus,category:'پیگیری',deadline:today(),due_time:'12:00',labels:['هتل'],collaborator_ids:[],created_at:nowIso()});setModal('task')}}/>} 
        ...
        {view==='approvals'&&can('approvals')&&<OpsModule kind='approvals' me={me} tasks={visibleTasks} hotels={hotels} users={users} logs={logs} reminders={reminders} activities={activities} saveTask={saveTask} openTask={(t:Task)=>openModal('task',t)} setView={setView}/>}
        {view==='sla'&&can('sla')&&<OpsModule kind='sla' me={me} tasks={visibleTasks} hotels={hotels} users={users} logs={logs} reminders={reminders} activities={activities} saveTask={saveTask} openTask={(t:Task)=>openModal('task',t)} setView={setView}/>}
        ...
        {view==='team'&&can('team')&&<Team users={users} roles={roles} tasks={tasks} activities={activities} logs={logs} profile={(u:User)=>openModal('employeeProfile',u)} edit={(u:User)=>openModal('user',u)} add={()=>openModal('user')} remove={(id:string)=>remove('ihos_users',id,()=>setUsers(users.filter(u=>u.id!==id)))}/>} 
...
function Sidebar({view,setView,can,settings,collapsed,toggle}:any){
  const sections:{title:string;items:[PermissionKey,string,any][]}[]=[
    {title:'میزکار',items:[['dashboard','داشبورد',LayoutDashboard],['inbox','میزکار من',ClipboardList],['executive','نمای مدیرعامل',Rocket]]},
    {title:'عملیات هتل',items:[['hotelSuperApp','سوپر اپ هتل',Hotel],['contracts','قراردادها و ریسک',FileText],['communications','ارتباطات هتل',Bell]]},
    {title:'تسک و جریان کار',items:[['tasks','تسک سنتر',ListChecks],['approvals','تأیید و بازبینی',CheckCircle2],['reminders','یادآورها و تقویم',AlarmClock],['messages','پیام داخلی',Bell],['sla','زمان‌بندی و SLA',Clock3],['playbooks','فرایندهای استاندارد',FolderKanban]]},
    {title:'تیم و پروژه',items:[['team','کارشناسان',Users],['projects','پروژه‌ها',FolderKanban],['goals','هدف‌گذاری',Target],['roles','نقش‌ها و دسترسی‌ها',Shield],['automations','اتوماسیون',Sparkles]]},
    {title:'داده و گزارش',items:[['reports','گزارش‌ساز',Activity],['dailyReport','گزارش روزانه',FileText],['savedViews','نماهای کاری',Eye],['bulkActions','عملیات گروهی',SlidersHorizontal],['logs','تاریخچه تغییرات',Archive],['documents','اسناد',FileText],['aiAssistant','دستیار هوشمند',Sparkles],['settings','تنظیمات',Settings]]},
  ]
  return <aside className={cn('sidebar v6Sidebar dynamicSidebar',collapsed&&'collapsed')}>
    <div className="brand"><div className="appLogo">{settings.logoUrl?<img src={settings.logoUrl}/>:<span>IH</span>}</div>{!collapsed&&<div className="brandCopy"><b>{settings.orgName}</b><small>Enterprise OS</small></div>}<button className="sidebarToggle" onClick={toggle} title={collapsed?'باز کردن منو':'جمع کردن منو'}>{collapsed?<ChevronLeft size={18}/>:<ChevronRight size={18}/>}</button></div>
    <div className="navScroll">{sections.map(sec=>{const its=sec.items.filter(([p])=>can(p)); if(!its.length) return null; return <section className="navSection" key={sec.title}>{!collapsed&&<p>{sec.title}</p>}{its.map(([id,label,Icon])=><button title={collapsed?label:undefined} key={id} className={view===id?'active':''} onClick={()=>setView(id)}><Icon size={18}/>{!collapsed&&<span>{label}</span>}</button>)}</section>})}</div>
    <button className="logoutSide" title={collapsed?'خروج':undefined} onClick={()=>{localStorage.removeItem('ihos-session');location.reload()}}><LogOut size={17}/>{!collapsed&&<span>خروج</span>}</button>
  </aside>
}
function Login({users,roles,setMe,online,syncAll,settings,registerUser}:any){
  ...
  if(mode==='success') return <div className="authPage"><div className="successPanel"><div className="authIllustration">📋</div><h2>ثبت‌نام با موفقیت انجام شد</h2><p>اکانت شما در IranHotel OS ساخته شد.</p><button className="btn primary" onClick={()=>setMe(users.find((u:User)=>u.username==='admin')||null)}>شروع</button></div></div>
...
function Team({users,roles,tasks,activities,logs,profile,edit,add,remove}:any){
  ...
  return <div className="employeesPage"><div className="employeesHead"><div><h2>Employees ({users.length})</h2><p className="muted">مدیریت کارشناسان، نقش‌ها، تیم‌ها و سطح دسترسی</p></div><div className="actions"><div className="seg"><button className={view==='list'?'on':''} onClick={()=>setView('list')}>List</button><button className={view==='activity'?'on':''} onClick={()=>setView('activity')}>Activity</button></div><button className="iconBtn"><Search/></button><button className="btn primary" onClick={add}><Plus/> Add Employee</button></div></div>{view==='activity'?<div className="employeeCards">{users.map((u:User,i:number)=><article className={cn('employeeCard',!u.is_active&&'sleep')} key={u.id}><div className="profileBand"><div className="avatar ring">{u.full_name.slice(0,1)}</div><h3>{u.full_name}</h3><p>{roleTitle(u)}</p><span>{u.zone||'عمومی'}</span></div><div className="employeeStats"><div><b>{i%3}</b><small>Backlog</small></div><div><b>{14+i}</b><small>In Progress</small></div><div><b>{(i+2)%7}</b><small>In Review</small></div></div><div className="actions center"><button className="btn primary" onClick={()=>profile(u)}><Eye/> پروفایل</button><button className="btn ghost" onClick={()=>edit(u)}><Edit3/> ویرایش</button><button className="iconBtn dangerBtn" onClick={()=>remove(u.id)}><Trash2/></button></div></article>)}</div>:<div className="employeeList">{users.map((u:User,i:number)=><div className="employeeRow" key={u.id}><div className="avatar mini">{u.full_name.slice(0,1)}</div><div><b>{u.full_name}</b><small>{u.email||u.username}</small></div><span>Gender<br/><b>—</b></span><span>Team<br/><b>{u.team||'—'}</b></span><span>Zone<br/><b>{u.zone||'—'}</b></span><span>Position<br/><b>{roleTitle(u)}</b></span><button className="iconBtn" onClick={()=>profile(u)}><Eye/></button><button className="iconBtn" onClick={()=>edit(u)}><MoreHorizontal/></button></div>)}</div>}<div className="profilePreview card"><div><h3>Employee’s Profile</h3><p>برای هر کاربر می‌توان نقش، تیم، زون، رمز، ایمیل و دسترسی را مدیریت کرد.</p></div><div className="profileMetrics"><span><b>{active}</b>فعال</span><span><b>{roles.length}</b>نقش</span><span><b>{users.length-active}</b>غیرفعال</span></div></div></div>
}
function CRM360({hotels,tasks,docs,users,setView,exportHotels,onCreateTask}:any){
  ...
  useEffect(()=>{
    try{const raw=localStorage.getItem('ihos-crm-stage-map');if(raw)setStageMap(JSON.parse(raw)||{})}catch{}
  ...
  const fallbackStage=(h:HotelT)=> Number(h.capacity_total||0)===0?'بدون ظرفیت':(h.contract_status||'').includes('تمدید')?'نیازمند تمدید':((h.risk_status||'').includes('ریسک')||(h.cooperation_status||'').includes('قطع'))?'نیازمند پیگیری':'فعال';
  ...
  async function moveHotel(lane:string){
    ...
    const next={...stageMap,[dragged.id]:lane};setStageMap(next);try{localStorage.setItem('ihos-crm-stage-map',JSON.stringify(next))}catch{}
    const db=await getSupabaseClient();
    if(db){const {error}=await db.from('ihos_hotels').update({crm_stage:lane,updated_at:nowIso()}).eq('id',dragged.id);if(error)toast('مرحله در مرورگر ذخیره شد؛ برای ذخیره در Supabase فایل SQL نسخه V12 را اجرا کن.')}
...
function HotelProfileModal({hotel,tasks,docs,events,logs,users,close,editHotel,newTask}:any){
...
function Modal({title,children,close,className}:{title:string;children:any;close:()=>void;className?:string}){return <div className="modalWrap"><div className={cn('modal',className)}><div className="modalHead"><div><span className="modalEyebrow">IRANHOTEL OPERATIONS</span><h2>{title}</h2></div><button className="iconBtn" onClick={close} aria-label="بستن"><X/></button></div>{children}</div></div>}
...
function HotelSearchPicker({value,onChange,seedHotels=[]}:any){
  ...
  return <div className="hotelPicker">
    <button type="button" className={cn('hotelPickerTrigger',open&&'active')} onClick={()=>setOpen(!open)}>
    ...
    {open&&<div className="hotelPickerPanel">
      <div className="hotelPickerSearch"><Search size={18}/><input autoFocus value={query} onChange={e=>setQuery(e.target.value)} placeholder="مثلاً درویشی، 1025 یا مشهد..."/></div>
      <div className="hotelPickerResults">
        <button type="button" className="hotelResult empty" onClick={()=>pick(undefined)}><span className="hotelResultAvatar">—</span><span><b>بدون هتل</b><small>تسک عمومی و بدون ارتباط با هتل</small></span></button>
        {loading&&<div className="hotelPickerLoading"><RefreshCw className="spin" size={18}/> در حال جستجو در تمام هتل‌ها...</div>}
        {!loading&&results.map(h=><button type="button" className={cn('hotelResult',h.id===value&&'selected')} key={h.id} onClick={()=>pick(h)}><span className="hotelResultAvatar">{h.title?.slice(0,1)||'H'}</span><span><b>{h.title}</b><small>{h.hotel_code||'بدون کد'} • {h.city||'بدون شهر'} • {h.provider||'بدون Provider'}</small></span>{h.id===value&&<CheckCircle2 size={18}/>}</button>)}
        {!loading&&query&&results.length===0&&<div className="hotelPickerEmpty">هتلی پیدا نشد. نام یا کد دیگری امتحان کن.</div>}
      </div>
    </div>}
  </div>
}
    ...
    <div className="taskComposer">
      <section className="taskComposerMain">
        ...
        <div className="taskGrid2">
          ...
          <div className="taskField"><label>دسته‌بندی</label><select value={t.category} onChange={e=>setT({...t,category:e.target.value})}>{categories.map((x:string)=><option key={x}>{x}</option>)}</select></div>
        ...
        <div className="taskSection activityEditor taskActivities"><div className="taskSectionHead"><div><h3>چک‌لیست فعالیت‌ها</h3><p>فعالیت‌های قابل اندازه‌گیری برای اجرای تسک</p></div><span>{Math.round(progressFor(t.id,acts))}٪</span></div><div className="addLine"><input value={newAct} onChange={e=>setNewAct(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addAct()} placeholder="یک فعالیت جدید بنویس..."/><button type="button" className="btn ghost" onClick={addAct}><Plus/> افزودن</button></div>{acts.map(a=><div className="actItem" key={a.id}><input type="checkbox" checked={a.is_done} onChange={e=>setActs(acts.map(x=>x.id===a.id?{...x,is_done:e.target.checked,done_at:e.target.checked?nowIso():undefined,done_by:t.assigned_to}:x))}/><input value={a.title} onChange={e=>setActs(acts.map(x=>x.id===a.id?{...x,title:e.target.value}:x))}/><input className="actMinutes" type="number" title="زمان تخمینی به دقیقه" value={a.estimated_minutes||0} onChange={e=>setActs(acts.map(x=>x.id===a.id?{...x,estimated_minutes:+e.target.value}:x))}/><button type="button" className="iconBtn dangerBtn" onClick={()=>setActs(acts.filter(x=>x.id!==a.id))}><Trash2 size={15}/></button></div>)}</div>
...
function RoleModal({role,close,save}:any){
  ...
  const groups:[string,PermissionKey[]][]=[
    ['داشبورد و گزارش',['dashboard','inbox','executive','reports','dailyReport','aiAssistant']],
    ['هتل و تأمین',['hotelSuperApp','crm360','hotels','hotels_import','contracts','communications','riskRadar']],
    ['تسک و جریان کار',['tasks','tasks_create','tasks_edit','tasks_delete','assignments','approvals','sla','playbooks','messages','reminders']],
    ['تیم و پروژه',['team','roles','projects','goals','automations']],
