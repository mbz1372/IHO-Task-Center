...
const ReviewCenterV17=dynamic(()=>import('@/components/enterprise/V17Modules').then(m=>m.ReviewCenterV17),{ssr:false});
const SlaCenterV17=dynamic(()=>import('@/components/enterprise/V17Modules').then(m=>m.SlaCenterV17),{ssr:false});
const MessageCenterV17=dynamic(()=>import('@/components/enterprise/V17Modules').then(m=>m.MessageCenterV17),{ssr:false});
const CalendarPlusV17=dynamic(()=>import('@/components/enterprise/V17Modules').then(m=>m.CalendarPlusV17),{ssr:false});
const FinancialContractsV17=dynamic(()=>import('@/components/enterprise/V17Modules').then(m=>m.FinancialContractsV17),{ssr:false});
const TeamCoverageV17=dynamic(()=>import('@/components/enterprise/V17Modules').then(m=>m.TeamCoverageV17),{ssr:false});
  ...
  return <div className={cn("appShell",sidebarCollapsed&&"sidebarCollapsed")}>
    ...
    <main className="main">
      ...
      <section className="content">
        ...
        {view==='team'&&can('team')&&<><Team users={users} roles={roles} tasks={tasks} activities={activities} logs={logs} profile={(u:User)=>openModal('employeeProfile',u)} edit={(u:User)=>openModal('user',u)} add={()=>openModal('user')} remove={(id:string)=>remove('ihos_users',id,()=>setUsers(users.filter(u=>u.id!==id)))}/><TeamCoverageV17 users={users} tasks={tasks}/></>} 
...
function Sidebar({view,setView,can,settings,collapsed,toggle}:any){
  const sections:{id:string;title:string;items:[PermissionKey,string,any][]}[]=[
    {id:'workspace',title:'میزکار',items:[['dashboard','داشبورد',LayoutDashboard],['inbox','میزکار من',ClipboardList],['executive','نمای مدیرعامل',Rocket]]},
    {id:'hotel',title:'عملیات هتل',items:[['hotelSuperApp','سوپر اپ هتل',Hotel],['crm360','چرخه و سلامت هتل',Activity],['contracts','قرارداد مالی و پرداخت',FileText],['communications','ارتباطات هتل',Bell]]},
    ...
    {id:'team',title:'تیم و پروژه',items:[['team','کارشناسان و نقشه پوشش',Users],['projects','پروژه‌ها',FolderKanban],['goals','هدف‌گذاری',Target],['roles','نقش‌ها و دسترسی‌ها',Shield],['automations','اتوماسیون',Sparkles]]},
    {id:'data',title:'داده و گزارش',items:[['reports','گزارش‌ساز',Activity],['dailyReport','گزارش روزانه',FileText],['savedViews','نماهای کاری',Eye],['bulkActions','عملیات گروهی',SlidersHorizontal],['logs','تاریخچه تغییرات',Archive],['documents','اسناد',FileText],['aiAssistant','دستیار هوشمند',Sparkles],['settings','تنظیمات',Settings]]},
  ];
  ...
  return <aside className={cn('sidebar v6Sidebar dynamicSidebar v17Sidebar',collapsed&&'collapsed')}>
    ...
    <div className="navScroll">{sections.map(sec=>{const its=sec.items.filter(([p])=>can(p));if(!its.length)return null;const expanded=open[sec.id]!==false;return <section className={cn('navSection v17NavSection',!expanded&&'closed')} key={sec.id}>{!collapsed&&<button className="navGroupHead" onClick={()=>toggleGroup(sec.id)}><span>{sec.title}</span><ChevronDown size={16}/></button>}{(collapsed||expanded)&&<div className="navGroupItems">{its.map(([id,label,Icon])=><button title={collapsed?label:undefined} key={id} className={view===id?'active':''} onClick={()=>setView(id)}><Icon size={18}/>{!collapsed&&<span>{label}</span>}</button>)}</div>}</section>})}</div>
...
function CRM360({hotels,tasks,docs,users,setView,exportHotels,onCreateTask}:any){
  ...
  const fallbackStage=(h:HotelT)=>(h.contract_status||'').includes('تمدید')?'نیازمند تمدید':((h.risk_status||'').includes('ریسک')||(h.cooperation_status||'').includes('قطع'))?'نیازمند پیگیری':(['IHO','Asa','Shab'].includes(String(h.provider||''))?'آنلاین‌سازی':'فعال');
