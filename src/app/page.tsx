...
export default function App(){
  ...
  async function saveTeam(next: ManagedUser[]){
    setTeamMembers(next);
    if(hasSupabase && supabase){ await Promise.all(next.map(m => supabase.from('iho_team_members').upsert({ id:m.id, payload:m }))); }
  }
  ...
  if(!logged) return <Login users={teamMembers.filter(u=>u.active!==false)} currentUser={currentUser} setCurrentUser={setCurrentUser} login={()=>setLogged(true)} />;
  ...
  return <main className="min-h-screen">
    ...
    <div className="max-w-[1500px] mx-auto px-4 py-5">
      <section className="hero card p-5 mb-4">
        <div>
          <p className="text-sm text-teal-700 font-black flex gap-2 items-center"><Sparkles size={18}/> IHO Task Center Super V3</p>
...
function Splash(){ return <div className="min-h-screen flex items-center justify-center"><div className="card p-8 text-center"><div className="mx-auto w-16 h-16 rounded-3xl bg-teal-50 flex items-center justify-center text-teal-700 mb-4"><Sparkles size={32}/></div><h1 className="text-2xl font-black">در حال آماده‌سازی کارتابل تأمین...</h1><p className="text-gray-500 mt-2">IHO Task Center Super</p></div></div>; }
function Login({users:members,currentUser,setCurrentUser,login}:any){ return <div className="min-h-screen flex items-center justify-center p-4 bgLogin"><div className="card p-7 w-full max-w-3xl"><div className="text-center mb-6"><div className="mx-auto w-16 h-16 rounded-3xl bg-teal-600 text-white flex items-center justify-center mb-4"><Building2 size={31}/></div><h1 className="text-3xl font-black">IHO Task Center Super</h1><p className="text-gray-500 mt-2">برای ورود، کاربر خود را انتخاب کن.</p></div><div className="grid md:grid-cols-2 gap-3">{members.map((u:ManagedUser)=><button key={u.id} onClick={()=>{setCurrentUser(u); login();}} className="loginUser"><div className="flex items-center gap-3"><div className="avatar">{u.name.slice(0,1)}</div><div className="text-right"><b>{u.name}</b><p className="text-sm text-gray-500">{roleMap[u.role]} | {u.team}</p></div></div>{u.role==='admin'&&<span className="badge" style={{background:'#dcfce7',color:'#166534'}}>دسترسی کامل</span>}</button>)}</div></div></div>; }
