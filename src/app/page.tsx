...
import { WorkforceCenter, WorkforceDepartment } from '@/components/team/WorkforceCenter';
...
type User={id:string;full_name:string;username:string;password_hash:string;role:string;role_id?:string;team?:string;zone?:string;mobile?:string;email?:string;avatar?:string;department_id?:string;department_name?:string;is_active:boolean;created_at?:string;updated_at?:string};
...
type Task={id:string;title:string;description?:string;hotel_id?:string;hotel_title?:string;city?:string;priority:string;status:string;category:string;assigned_to?:string;assigned_name?:string;department_id?:string;department_name?:string;created_by?:string;deadline?:string;due_time?:string;completed_at?:string;created_at?:string;updated_at?:string;labels?:string[];collaborator_ids?:string[];project_id?:string;pinned_note?:string;estimated_minutes?:number;spent_minutes?:number;source_type?:string;source_id?:string;automation_id?:string};
...
const initialDepartments:WorkforceDepartment[]=[{id:'dept-supply',name:'تأمین و عملیات',description:'مدیریت تأمین، ظرفیت، نرخ و عملیات هتل‌ها',color:'#2563eb',manager_id:'u-admin',is_active:true}];
const initialUsers:User[]=[{id:'u-admin',full_name:'محمدباقر ذوالفقاری',username:'admin',password_hash:'123456',role:'مدیر سیستم',role_id:'role-admin',team:'مدیریت تأمین',zone:'کل کشور',department_id:'dept-supply',department_name:'تأمین و عملیات',is_active:true},{id:'u-1',full_name:'فاطمه رنجبر',username:'ranjbar',password_hash:'123456',role:'کارشناس',role_id:'role-expert',team:'شرق و جنوب',zone:'شرق و جنوب',department_id:'dept-supply',department_name:'تأمین و عملیات',is_active:true},{id:'u-2',full_name:'پگاه واعظین',username:'vaezin',password_hash:'123456',role:'کارشناس',role_id:'role-expert',team:'شمال و غرب',zone:'شمال و غرب',department_id:'dept-supply',department_name:'تأمین و عملیات',is_active:true},{id:'u-3',full_name:'فائزه سالاری',username:'salari',password_hash:'123456',role:'سیتی منیجر',role_id:'role-city',team:'مرکز',zone:'مرکز',department_id:'dept-supply',department_name:'تأمین و عملیات',is_active:true}];
...
function IHOSApp(){
  ...
  const [departments,setDepartments]=useLocal<WorkforceDepartment[]>('ihos-departments',initialDepartments);
  ...
  const safeDepartments = Array.isArray(departments) ? departments : initialDepartments;
  ...
  useEffect(()=>{
    ...
    const refreshMap:Record<string,()=>void>={
      ...
      ihos_departments:()=>syncTable('ihos_departments',setDepartments),
    ...
    getSupabaseClient().then(db=>{
      ...
      channel=db.channel('ihos-live-v64')
        ...
        .on('postgres_changes',{event:'*',schema:'public',table:'ihos_departments'},()=>refreshMap.ihos_departments())
  ...
  async function saveDepartment(department:WorkforceDepartment){await saveGeneric('ihos_departments',department,departments,setDepartments,'department',department.name)}
  async function importExperts(file:File){
    ...
    const rows:User[]=data.map((raw,index)=>{
      ...
      const departmentText=pick(raw,['دپارتمان','واحد سازمانی','department']);
      const department=departments.find(item=>norm(item.name)===norm(departmentText))||departments.find(item=>item.id===previous?.department_id)||departments[0];
      if(!fullName)return null as any;
      return {...previous,id:previous?.id||uid(),full_name:fullName,username,password_hash:pick(raw,['رمز عبور','password'])||previous?.password_hash||'123456',role:role?.title||roleText,role_id:role?.id||'role-expert',department_id:department?.id,department_name:department?.name,team:pick(raw,['تیم','team'])||previous?.team||'',zone:pick(raw,['منطقه','زون','استان','zone','region'])||previous?.zone||'',mobile:mobile||previous?.mobile||'',email:email||previous?.email||'',is_active:!['خیر','غیرفعال','false','0'].includes(norm(pick(raw,['فعال','وضعیت','active']))),created_at:previous?.created_at||nowIso(),updated_at:nowIso()};
    }).filter(Boolean);
  ...
  async function importHotelAssignments(file:File){
    ...
    data.forEach(row=>{const hotelName=pick(row,['نام هتل','hotel','hotel_name','hotel title']),expertName=pick(row,['First name','نام کارشناس','کارشناس','expert','assignee','مسئول']),taskId=Number(pick(row,['taskId','task id','task_id','TaskId','نوع تسک'])),assignmentRole=taskId===1?'capacity_expert':taskId===2?'rate_expert':'account_manager';if(!hotelName||!expertName)return;const hotel=findHotel(hotelName),user=findUser(expertName);if(!hotel){missingHotels.push(hotelName);return}if(!user){missingUsers.push(expertName);return}map.set(`${hotel.id}-${assignmentRole}`,{id:`assignment-import-${hotel.id}-${assignmentRole}`,hotel_id:hotel.id,hotel_title:hotel.title,user_id:user.id,user_name:user.full_name,assignment_role:assignmentRole,is_primary:true,active:true,started_at:today(),created_at:nowIso(),updated_at:nowIso()})});
    ...
    const automationMap=new Map<string,any>();rows.filter(r=>['rate_expert','capacity_expert'].includes(r.assignment_role)).forEach(r=>{const hotel=allHotels.find(h=>h.id===r.hotel_id),automation=automationMap.get(r.hotel_id)||{id:`automation-${r.hotel_id}`,hotel_id:r.hotel_id,hotel_code:hotel?.hotel_code||null,provider:'IHO Provider',updated_at:nowIso()};automation[r.assignment_role]=r.user_name;automationMap.set(r.hotel_id,automation)});const automationRows=[...automationMap.values()];for(let i=0;i<automationRows.length;i+=250){const {error}=await db.from('ihos_hotel_automation').upsert(automationRows.slice(i,i+250),{onConflict:'id'});if(error)throw error}
    let ownershipWarning='';try{const ids=[...new Set(rows.map(r=>r.hotel_id))];for(let i=0;i<ids.length;i+=250){const {error}=await db.from('ihos_hotel_assignments').update({active:false,ended_at:today(),updated_at:nowIso()}).in('hotel_id',ids.slice(i,i+250)).in('assignment_role',['rate_expert','capacity_expert','account_manager']).eq('active',true);if(error)throw error}for(let i=0;i<rows.length;i+=250){const {error}=await db.from('ihos_hotel_assignments').upsert(rows.slice(i,i+250),{onConflict:'id'});if(error)throw error}}catch{ownershipWarning='تاریخچه کامل مالکیت پس از اجرای migration نسخه ۲۲ فعال می‌شود'}
    ...
    const notes=[`${rows.length.toLocaleString('fa-IR')} تخصیص اعمال شد`];if(missingHotels.length)notes.push(`${missingHotels.length.toLocaleString('fa-IR')} نام هتل تطبیق نداشت`);if(missingUsers.length)notes.push(`${missingUsers.length.toLocaleString('fa-IR')} نام کارشناس تطبیق نداشت`);if(ownershipWarning)notes.push(ownershipWarning);toast(notes.join(' · '));
  }
  ...
  return <div className={cn("appShell",sidebarCollapsed&&"sidebarCollapsed")}>
    ...
    <main className="main" aria-busy={busy}>
      ...
      <section className="content" id="main-content" tabIndex={-1} aria-label="محتوای اصلی">
        ...
        {view==='tasks'&&can('tasks')&&<Tasks tasks={filteredTasks} users={users} departments={safeDepartments} statuses={statuses} activities={activities} can={can} saveTask={(t:Task)=>saveTask(t)} edit={(t:Task)=>openModal('task',t)} remove={(id:string)=>remove('ihos_tasks',id,()=>setTasks(tasks.filter(t=>t.id!==id)))} openDetails={(t:Task)=>openModal('task',t)}/>}
        ...
        {view==='team'&&can('team')&&<><ExpertImporter run={importExperts} runAssignments={importHotelAssignments}/><WorkforceCenter users={users} roles={roles} tasks={tasks} departments={safeDepartments} profile={(u:any)=>openModal('employeeProfile',u)} edit={(u:any)=>openModal('user',u)} add={()=>openModal('user')} remove={(id:string)=>remove('ihos_users',id,()=>setUsers(users.filter(u=>u.id!==id)))} addDepartment={()=>openModal('department')} editDepartment={(department:WorkforceDepartment)=>openModal('department',department)}/><TeamCoverageV17 users={users} tasks={tasks} hotels={hotels}/></>}
    ...
    {modal==='task'&&<TaskModal task={editing} users={users} departments={safeDepartments} hotels={hotels} projects={projects} settings={settings} labels={labels} statuses={statuses} categories={categories} activities={activities.filter(a=>a.task_id===editing?.id)} close={()=>setModal(null)} save={saveTask}/>}
    {modal==='user'&&<UserModal user={editing} roles={roles} departments={safeDepartments} uploadFile={(file:File)=>uploadFile(file,'experts')} close={()=>setModal(null)} save={saveUser}/>}
    {modal==='department'&&<DepartmentModal department={editing} users={users} close={()=>setModal(null)} save={saveDepartment}/>}
...
function Sidebar({view,setView,can,settings,collapsed,toggle}:any){
  ...
  return <aside className={cn('sidebar v6Sidebar dynamicSidebar v17Sidebar',collapsed&&'collapsed')}>
    <div className="brand"><BrandLogo settings={settings}/>{!collapsed&&<div className="brandCopy"><b>{settings.orgName}</b><small>Enterprise OS</small></div>}<button className="sidebarToggle" onClick={toggle} title={collapsed?'باز کردن منو':'جمع کردن منو'} aria-label={collapsed?'باز کردن منو':'جمع کردن منو'}>{collapsed?<ChevronLeft size={18}/>:<ChevronRight size={18}/>}</button></div>
...
function BrandLogo({settings,big=false}:{settings:AppSettings;big?:boolean}){const [failed,setFailed]=useState(false);useEffect(()=>setFailed(false),[settings.logoUrl]);return <div className={cn('appLogo',big&&'big')}>{settings.logoUrl&&!failed?<img src={settings.logoUrl} alt={`نشان ${settings.orgName}`} onError={()=>setFailed(true)}/>:<span aria-hidden="true">IH</span>}</div>}
function ProfileAvatar({user,className=''}:{user?:User;className?:string}){const [failed,setFailed]=useState(false);useEffect(()=>setFailed(false),[user?.avatar]);return <div className={cn('avatar',className,user?.avatar&&!failed&&'hasImage')}>{user?.avatar&&!failed?<img src={user.avatar} alt={`تصویر ${user.full_name}`} onError={()=>setFailed(true)}/>:user?.full_name?.slice(0,1)||'U'}</div>}
...
function Tasks({tasks,users,departments,statuses,activities,can,saveTask,edit,remove}:any){
  ...
  const list=tasks.filter((task:Task)=>(statusFilter==='همه'||task.status===statusFilter)&&(departmentFilter==='همه'||task.department_id===departmentFilter));
...
function TaskModal({task,users,departments,hotels,projects,settings,labels,statuses,categories,activities,close,save}:any){
  ...
  const eligibleUsers=t.department_id?users.filter((user:User)=>user.department_id===t.department_id):users;
  function setDepartment(id:string){const department=departments.find((item:WorkforceDepartment)=>item.id===id);const currentUser=users.find((user:User)=>user.id===t.assigned_to);const keepAssignee=!id||currentUser?.department_id===id;setT({...t,department_id:id||undefined,department_name:department?.name,assigned_to:keepAssignee?t.assigned_to:undefined,assigned_name:keepAssignee?t.assigned_name:undefined})}
  function setUser(id:string){const u=users.find((x:User)=>x.id===id);setT({...t,assigned_to:id||undefined,assigned_name:u?.full_name,department_id:u?.department_id||t.department_id,department_name:u?.department_name||departments.find((department:WorkforceDepartment)=>department.id===u?.department_id)?.name||t.department_name})}
    ...
    <div className="taskComposer">
      <section className="taskComposerMain">
        ...
        <div className="taskGrid2">
          <div className="taskField"><label>دپارتمان مسئول</label><select value={t.department_id||''} onChange={e=>setDepartment(e.target.value)}><option value="">بدون دپارتمان</option>{departments.filter((department:WorkforceDepartment)=>department.is_active).map((department:WorkforceDepartment)=><option key={department.id} value={department.id}>{department.name}</option>)}</select></div>
          <div className="taskField"><label>مسئول اصلی</label><select value={t.assigned_to||''} onChange={e=>setUser(e.target.value)}><option value="">انتخاب مسئول</option>{eligibleUsers.filter((u:User)=>u.is_active).map((u:User)=><option key={u.id} value={u.id}>{u.full_name} — {u.team||u.role}</option>)}</select></div>
...
function UserModal({user,roles,departments,uploadFile,close,save}:any){
  const [u,setU]=useState<User>(user||{id:uid(),full_name:'',username:'',password_hash:'123456',role:'کارشناس',role_id:'role-expert',team:'',zone:'',mobile:'',email:'',department_id:departments[0]?.id,department_name:departments[0]?.name,is_active:true,created_at:nowIso()});
  const [uploading,setUploading]=useState(false);
  function setRole(id:string){const r=roles.find((x:Role)=>x.id===id);setU({...u,role_id:id,role:r?.title||u.role})}
  function setDepartment(id:string){const department=departments.find((item:WorkforceDepartment)=>item.id===id);setU({...u,department_id:id||undefined,department_name:department?.name})}
  async function setPhoto(file?:File){if(!file)return;if(!file.type.startsWith('image/')){toast('فقط فایل تصویری انتخاب کن');return}if(file.size>3*1024*1024){toast('حجم تصویر باید کمتر از ۳ مگابایت باشد');return}setUploading(true);try{const avatar=await uploadFile(file);setU(current=>({...current,avatar}))}finally{setUploading(false)}}
  return <Modal title={user?'ویرایش کاربر':'افزودن کاربر جدید'} close={close} className="userModalV22"><div className="userPhotoEditorV22"><ProfileAvatar user={u} className="xl"/><div><b>تصویر کارشناس</b><span>JPG، PNG یا WebP تا ۳ مگابایت</span><label className="btn ghost">{uploading?'در حال آپلود...':'انتخاب تصویر'}<input type="file" accept="image/jpeg,image/png,image/webp" disabled={uploading} onChange={event=>void setPhoto(event.target.files?.[0])}/></label>{u.avatar&&<button type="button" className="linkBtn dangerText" onClick={()=>setU({...u,avatar:undefined})}>حذف تصویر</button>}</div></div><div className="form"><Field label="نام کامل" value={u.full_name} onChange={(v:string)=>setU({...u,full_name:v})}/><Field label="نام کاربری" value={u.username} onChange={(v:string)=>setU({...u,username:v})}/><Field label="رمز عبور" value={u.password_hash} onChange={(v:string)=>setU({...u,password_hash:v})}/><div><label>نقش</label><select value={u.role_id||''} onChange={e=>setRole(e.target.value)}>{roles.map((r:Role)=><option key={r.id} value={r.id}>{r.title}</option>)}</select></div><div><label>دپارتمان</label><select value={u.department_id||''} onChange={e=>setDepartment(e.target.value)}><option value="">بدون دپارتمان</option>{departments.filter((department:WorkforceDepartment)=>department.is_active).map((department:WorkforceDepartment)=><option key={department.id} value={department.id}>{department.name}</option>)}</select></div><Field label="تیم" value={u.team} onChange={(v:string)=>setU({...u,team:v})}/><Field label="زون" value={u.zone} onChange={(v:string)=>setU({...u,zone:v})}/><Field label="موبایل" value={u.mobile} onChange={(v:string)=>setU({...u,mobile:v})}/><Field label="ایمیل" value={u.email} onChange={(v:string)=>setU({...u,email:v})}/><label className="settingToggleV15 full"><input type="checkbox" checked={u.is_active} onChange={e=>setU({...u,is_active:e.target.checked})}/><i/><span><b>کارشناس فعال است</b><small>در فهرست تخصیص تسک‌ها نمایش داده شود</small></span></label><button className="btn primary full" disabled={uploading||!u.full_name.trim()||!u.username.trim()} onClick={()=>save(u)}><Save/> ذخیره کاربر</button></div></Modal>
}

function DepartmentModal({department,users,close,save}:any){
  const [item,setItem]=useState<WorkforceDepartment>(department||{id:uid(),name:'',description:'',color:'#2563eb',manager_id:'',is_active:true});
  return <Modal title={department?'ویرایش دپارتمان':'دپارتمان جدید'} close={close} className="departmentModalV22"><div className="departmentModalIntroV22"><Building2/><div><h3>ساختار سازمانی قابل توسعه</h3><p>کارشناسان و تسک‌های هر واحد را از یک نقطه مدیریت کن.</p></div></div><div className="form"><Field label="نام دپارتمان" value={item.name} onChange={(value:string)=>setItem({...item,name:value})}/><Field label="رنگ سازمانی" type="color" value={item.color} onChange={(value:string)=>setItem({...item,color:value})}/><div><label>مدیر دپارتمان</label><select value={item.manager_id||''} onChange={event=>setItem({...item,manager_id:event.target.value||undefined})}><option value="">بدون مدیر</option>{users.filter((user:User)=>user.is_active).map((user:User)=><option key={user.id} value={user.id}>{user.full_name}</option>)}</select></div><label className="settingToggleV15"><input type="checkbox" checked={item.is_active} onChange={event=>setItem({...item,is_active:event.target.checked})}/><i/><span><b>دپارتمان فعال</b><small>در فرم کاربران و تسک‌ها نمایش داده شود</small></span></label><div className="full"><label>توضیح</label><textarea value={item.description||''} onChange={event=>setItem({...item,description:event.target.value})} placeholder="حوزه مسئولیت و مأموریت این دپارتمان..."/></div><button className="btn primary full" disabled={!item.name.trim()} onClick={()=>save(item)}><Save/> ذخیره دپارتمان</button></div></Modal>
}
