...
function Dashboard({visible,tasks,kpi,openTask,teamMembers,openCreate,setTab}:any){
  const latest=[...visible].sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt)).slice(0,7);
  const hotHotels=[...new Set(tasks.filter((t:Task)=>activeStatuses.includes(t.status)).map((t:Task)=>t.hotelName))].slice(0,8);
  const todayTasks=visible.filter((t:Task)=>t.dueDate===today());
  const doneRate=visible.length?Math.round(visible.filter((t:Task)=>t.status==='انجام‌شده').length/visible.length*100):0;
  const teamLoad=teamMembers.filter((m:ManagedUser)=>m.active!==false && m.role!=='admin').map((m:ManagedUser)=>({m, count:tasks.filter((t:Task)=>t.assignee===m.name && activeStatuses.includes(t.status)).length})).sort((a,b)=>b.count-a.count).slice(0,6);
  const urgentHotels=visible.filter((t:Task)=>t.priority==='فوری'||isOverdue(t)).slice(0,6);
  return <><section className="gridDash mb-4"><Kpi title="کل تسک‌ها" value={kpi.all} icon={<LayoutDashboard/>} accent/><Kpi title="باز" value={kpi.open} icon={<Clock3/>}/><Kpi title="امروز" value={todayTasks.length} icon={<CalendarDays/>}/><Kpi title="عقب‌افتاده" value={kpi.late} icon={<CalendarDays/>} danger/><Kpi title="فوری" value={kpi.urgent} icon={<Flame/>} danger/><Kpi title="انجام‌شده" value={kpi.done} icon={<CheckCircle2/>} accent/></section><section className="grid lg:grid-cols-12 gap-4"><div className="card p-5 lg:col-span-8 commandCard"><div className="flex justify-between mb-4 gap-3 flex-wrap"><div><h2 className="font-black text-xl">Command Center</h2><p className="text-gray-500 text-sm mt-1">نمای مدیریتی برای کنترل فوری تیم تأمین</p></div><button className="btn" onClick={()=>openCreate()}><Plus size={16}/> تسک سریع</button></div><div className="grid md:grid-cols-4 gap-3 mb-4"><ActionBox title="نرخ تکمیل" value={doneRate+'%'} text="از کل تسک‌ها"/><ActionBox title="هتل‌های درگیر" value={hotHotels.length} text="با تسک باز"/><ActionBox title="منتظر پاسخ هتل" value={kpi.waiting} text="نیازمند یادآوری"/><ActionBox title="تایید مدیر" value={kpi.approval} text="در صف بررسی"/></div><h3 className="font-black mb-3">آخرین تغییرات</h3>{latest.length?latest.map((t:Task)=><button key={t.id} onClick={()=>openTask(t)} className="activityRow"><div><b>{t.title}</b><p>{t.hotelName} | {t.assignee} | {faDate(t.updatedAt)}</p></div><span className="badge" style={statusColor[t.status]}>{t.status}</span></button>):<p className="text-gray-500">هنوز تسکی ثبت نشده است.</p>}</div><div className="lg:col-span-4 space-y-4"><div className="card p-5 insightCard"><h2 className="font-black text-xl mb-3 flex gap-2 items-center"><Sparkles/> پیشنهادهای مدیریتی</h2>{urgentHotels.length?<div className="space-y-2">{urgentHotels.map((t:Task)=><button key={t.id} onClick={()=>openTask(t)} className="miniAlert"><b>{t.hotelName}</b><span>{isOverdue(t)?'عقب‌افتاده':'فوری'}</span></button>)}</div>:<p className="text-gray-500 leading-8">وضعیت کارتابل سالم است. تسک فوری یا عقب‌افتاده مهمی دیده نمی‌شود.</p>}<button className="btn secondary w-full mt-4" onClick={()=>setTab('kanban')}>رفتن به کارتابل</button></div><div className="card p-5"><h2 className="font-black text-xl mb-4">بار کاری تیم</h2>{teamLoad.map(({m,count}:any)=>{const target=m.dailyTarget||8;const rate=Math.min(100,Math.round(count/target*100));return <div key={m.id} className="mb-4"><div className="flex justify-between text-sm"><span>{m.name}</span><b>{count}/{target}</b></div><div className="progress"><span style={{width:rate+'%'}}/></div></div>})}<button className="btn secondary w-full mt-2" onClick={()=>setTab('reports')}>گزارش کامل</button></div></div></section></>;
}
...
function AdminTeamPage({members,setMembers,tasks}:{members:ManagedUser[];setMembers:(m:ManagedUser[])=>Promise<void>|void;tasks:Task[]}){
 const blank: ManagedUser = { id:'u-'+Date.now(), name:'', role:'expert', team:'مرکز', active:true, phone:'', title:'', dailyTarget:8 }; const [editing,setEditing]=useState<(ManagedUser & {newPassword?: string})|null>(null); const [search,setSearch]=useState(''); const [roleFilter,setRoleFilter]=useState<'all'|Role>('all'); const [saving,setSaving]=useState(false); const zones=[...new Set(members.map(m=>m.team).filter(Boolean))]; const list=members.filter(m=>(roleFilter==='all'||m.role===roleFilter)).filter(m=>!search || [m.name,m.team,m.role,m.phone,m.title].join(' ').includes(search)); const zoneStats=zones.map(z=>({z, members:members.filter(m=>m.team===z).length, open:tasks.filter(t=>members.filter(m=>m.team===z).map(m=>m.name).includes(t.assignee)&&activeStatuses.includes(t.status)).length})); async function save(){ if(!editing?.name.trim()) return alert('نام کارشناس را وارد کن'); setSaving(true); try{ const exists=members.some(m=>m.id===editing.id); const next=exists?members.map(m=>m.id===editing.id?editing:m):[{...editing,id:editing.id||'u-'+uid()},...members]; await setMembers(next as ManagedUser[]); setEditing(null); } finally { setSaving(false); } } async function toggle(id:string){ await setMembers(members.map(m=>m.id===id?{...m,active:m.active===false}:m)); } async function remove(id:string){ if(confirm('این عضو حذف شود؟ این عملیات در Supabase هم ذخیره می‌شود.')) await setMembers(members.filter(m=>m.id!==id)); } async function activateAll(){ if(confirm('همه اعضا فعال شوند؟')) await setMembers(members.map(m=>({...m,active:true}))); }
 return <section className="space-y-4"><div className="card p-5 adminHero"><div className="flex flex-wrap justify-between gap-3 items-center"><div><h2 className="text-2xl font-black flex items-center gap-2"><ShieldCheck/> مدیریت سازمانی تیم</h2><p className="text-gray-500 mt-1">مدیریت اعضا، نقش‌ها، زون‌ها، هدف روزانه، رمز عبور و عملکرد هر نفر.</p></div><div className="flex gap-2 flex-wrap"><button className="btn secondary" onClick={activateAll}>فعال‌سازی همه</button><button className="btn" onClick={()=>setEditing(blank)}><UserPlus size={17}/> افزودن عضو</button></div></div></div><section className="grid md:grid-cols-4 gap-3"><Kpi title="کل اعضا" value={members.length} icon={<Users/>}/><Kpi title="فعال" value={members.filter(m=>m.active!==false).length} icon={<CheckCircle2/>}/><Kpi title="زون‌ها" value={zones.length} icon={<Building2/>}/><Kpi title="کارشناس‌ها" value={members.filter(m=>!['admin','manager'].includes(m.role)).length} icon={<ListTodo/>}/></section><div className="grid lg:grid-cols-3 gap-4"><div className="card p-4 lg:col-span-2"><div className="grid md:grid-cols-3 gap-3 mb-4"><input className="input md:col-span-2" placeholder="جستجوی نام، تیم، نقش، شماره..." value={search} onChange={e=>setSearch(e.target.value)}/><select className="input" value={roleFilter} onChange={e=>setRoleFilter(e.target.value as any)}><option value="all">همه نقش‌ها</option>{Object.entries(roleMap).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></div><div className="tableWrap"><table className="tbl"><thead><tr><th>نام</th><th>سمت</th><th>نقش</th><th>تیم/زون</th><th>تلفن</th><th>هدف</th><th>KPI</th><th>وضعیت</th><th>عملیات</th></tr></thead><tbody>{list.map(m=>{const mt=tasks.filter(t=>t.assignee===m.name||t.manager===m.name||t.createdBy===m.name);const done=mt.filter(t=>t.status==='انجام‌شده').length;const late=mt.filter(isOverdue).length;const open=mt.filter(t=>activeStatuses.includes(t.status)).length;return <tr key={m.id}><td><b>{m.name}</b></td><td>{m.title||'-'}</td><td>{roleMap[m.role]}</td><td>{m.team}</td><td>{m.phone||'-'}</td><td>{m.dailyTarget||'-'}</td><td><span className="badge" style={{background:'#f1f5f9'}}>باز {open}</span> <span className="badge" style={{background:'#dcfce7'}}>انجام {done}</span> <span className="badge" style={{background:'#fee2e2'}}>عقب {late}</span></td><td><span className="badge" style={{background:m.active===false?'#fee2e2':'#dcfce7',color:m.active===false?'#991b1b':'#166534'}}>{m.active===false?'غیرفعال':'فعال'}</span></td><td><div className="flex gap-2"><button className="btn secondary" onClick={()=>setEditing({...m,newPassword:''})}><Edit3 size={15}/></button><button className="btn secondary" onClick={()=>toggle(m.id)}>{m.active===false?'فعال':'غیرفعال'}</button>{m.role!=='admin'&&<button className="btn danger" onClick={()=>remove(m.id)}><Trash2 size={15}/></button>}</div></td></tr>})}</tbody></table></div></div><div className="card p-4"><h3 className="font-black text-lg mb-3">نمای زون‌ها</h3>{zoneStats.map(z=><div key={z.z} className="zoneRow"><div><b>{z.z}</b><p>{z.members} عضو</p></div><span>{z.open} تسک باز</span></div>)}<div className="rounded-3xl bg-slate-50 p-4 mt-4 text-sm text-gray-500 leading-8"><b className="text-slate-700">نکته مدیریتی:</b><br/>برای هر کارشناس هدف روزانه بگذار تا در داشبورد، بار کاری دقیق‌تر دیده شود.</div></div></div>{editing&&<div className="modalBack"><div className="modal p-5"><div className="flex justify-between mb-4"><h2 className="font-black text-xl">{editing.id.startsWith('u-') && !members.some(m=>m.id===editing.id)?'افزودن عضو':'ویرایش عضو'}</h2><button className="btn secondary" onClick={()=>setEditing(null)}><X/></button></div><div className="grid md:grid-cols-2 gap-3"><input className="input" placeholder="نام" value={editing.name} onChange={e=>setEditing({...editing,name:e.target.value})}/><input className="input" placeholder="سمت" value={editing.title||''} onChange={e=>setEditing({...editing,title:e.target.value})}/><select className="input" value={editing.role} onChange={e=>setEditing({...editing,role:e.target.value as Role})}>{Object.entries(roleMap).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select><input className="input" placeholder="تیم/زون" value={editing.team} onChange={e=>setEditing({...editing,team:e.target.value})}/><input className="input" placeholder="شماره تماس" value={editing.phone||''} onChange={e=>setEditing({...editing,phone:e.target.value})}/><input className="input" type="number" placeholder="هدف روزانه" value={editing.dailyTarget||0} onChange={e=>setEditing({...editing,dailyTarget:Number(e.target.value)})}/><input className="input md:col-span-2" type="password" placeholder="رمز جدید؛ خالی بماند یعنی تغییر نکند" value={editing.newPassword||''} onChange={e=>setEditing({...editing,newPassword:e.target.value})}/></div><button className="btn mt-4" onClick={save} disabled={saving}>{saving?'در حال ذخیره...':'ذخیره تغییرات'}</button></div></div>}</section>;
}
function SettingsPage({syncMode,tasks,setTasks}:any){ const [copied,setCopied]=useState(false); function downloadJson(){ const blob=new Blob([JSON.stringify(tasks,null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='iho-tasks-backup.json'; a.click(); URL.revokeObjectURL(a.href); } function copyJson(){ navigator.clipboard?.writeText(JSON.stringify(tasks,null,2)); setCopied(true); setTimeout(()=>setCopied(false),1500); } function cleanupDone(){ if(confirm('تسک‌های انجام‌شده از نمای فعلی پاک شود؟')) setTasks(tasks.filter((t:Task)=>t.status!=='انجام‌شده')); } return <section className="space-y-4"><div className="card p-5 adminHero"><h2 className="font-black text-2xl flex gap-2 items-center"><Settings/> مرکز تنظیمات سیستم</h2><p className="text-gray-500 mt-1">کنترل وضعیت آنلاین، پشتیبان‌گیری، قوانین عملیاتی و ابزارهای مدیریتی.</p></div><section className="grid md:grid-cols-3 gap-4"><div className="card p-5"><h3 className="font-black mb-3">وضعیت اتصال</h3><p className="text-gray-500">حالت ذخیره‌سازی: <b>{syncMode==='supabase'?'Supabase Online':'LocalStorage'}</b></p><p className="text-gray-500 mt-2">تعداد تسک‌ها: <b>{tasks.length}</b></p><span className="badge mt-4" style={{background:syncMode==='supabase'?'#dcfce7':'#fef3c7',color:syncMode==='supabase'?'#166534':'#92400e'}}>{syncMode==='supabase'?'Online':'Local'}</span></div><div className="card p-5"><h3 className="font-black mb-3">پشتیبان‌گیری</h3><div className="grid gap-2"><button className="btn secondary" onClick={downloadJson}><Download size={16}/> دانلود Backup JSON</button><button className="btn secondary" onClick={copyJson}>{copied?'کپی شد':'کپی JSON'}</button></div></div><div className="card p-5"><h3 className="font-black mb-3">Danger Zone</h3><div className="grid gap-2"><button className="btn danger" onClick={cleanupDone}>پاکسازی انجام‌شده‌ها</button><button className="btn danger" onClick={()=>{if(confirm('دیتای محلی ریست شود؟')){localStorage.removeItem('iho_tasks_v3');setTasks(seedTasks.map(normalizeTask));}}}>ریست دیتای محلی</button></div></div></section><section className="grid lg:grid-cols-2 gap-4"><div className="card p-5"><h3 className="font-black text-lg mb-3">وضعیت‌های سیستم</h3><div className="flex flex-wrap gap-2">{statuses.map(s=><span key={s} className="badge" style={statusColor[s]}>{s}</span>)}</div><p className="text-gray-500 text-sm mt-4 leading-7">این وضعیت‌ها در Kanban و گزارش‌ها استفاده می‌شوند. در نسخه بعدی مدیریت کامل وضعیت‌ها دیتابیسی می‌شود.</p></div><div className="card p-5"><h3 className="font-black text-lg mb-3">اولویت‌ها</h3><div className="flex flex-wrap gap-2">{priorities.map(p=><span key={p} className="badge" style={priorityColor[p]}>{p}</span>)}</div><p className="text-gray-500 text-sm mt-4 leading-7">تسک‌های فوری و عقب‌افتاده در داشبورد مدیریتی به عنوان هشدار نمایش داده می‌شوند.</p></div></section><div className="card p-5"><h2 className="font-black text-xl mb-3">راهنمای محصول داخلی سازمانی</h2><p className="text-gray-500 leading-8">برای استفاده واقعی تیم تأمین: ورود با رمز، مدیریت اعضا از پنل مدیر، ثبت تسک‌ها روی Supabase، کنترل روزانه از داشبورد، و گزارش هفتگی برای مدیر زنجیره تأمین.</p></div></section>; }
