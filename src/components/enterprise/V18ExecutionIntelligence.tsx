...
export function HotelOwnershipV18({seedHotels=[],users=[],initialHotel}:any){
  ...
  useEffect(()=>{void loadAssignments().then(setRows).catch(e=>setMsg(`دریافت مسئولیت‌ها ناموفق بود: ${e.message}`))},[]);
  ...
  async function save(){
    ...
    try{
      ...
      setRows(next);setMsg('مسئولیت‌های هتل در Supabase ذخیره شد');setSelected(null);
