...
function writeLocal(key:string,value:any){try{localStorage.setItem(key,JSON.stringify(value))}catch{}}

async function loadAssignments(){
  try{return await loadAll('ihos_hotel_assignments','*') as Assignment[]}
  catch{return readLocal<Assignment[]>('ihos-v18-assignments',[])}
}

function useFullHotels(seed:any[]){
  const [hotels,setHotels]=useState<any[]>(seed||[]),[loading,setLoading]=useState(false);
  useEffect(()=>{let live=true;(async()=>{const db=getSupabase();if(!db)return;setLoading(true);try{const rows=await loadAll('ihos_hotels','id,hotel_code,title,city,province,provider,capacity_total,contract_status,contract_date,status_end_date,cooperation_status,risk_status,site_visible,search_visible,hotel_category,updated_at');if(live&&rows.length)setHotels(rows)}catch{}finally{if(live)setLoading(false)}})();return()=>{live=false}},[]);
...
export function ControlTowerV18({seedHotels=[],tasks=[],users=[],onCreateTask,onOpenOwnership}:any){
  const {hotels,loading}=useFullHotels(seedHotels);
  const [assignments,setAssignments]=useState<Assignment[]>([]),[q,setQ]=useState(''),[severity,setSeverity]=useState('all'),[reason,setReason]=useState('all');
  useEffect(()=>{void loadAssignments().then(setAssignments)},[]);
...
export function HotelOwnershipV18({seedHotels=[],users=[],initialHotel}:any){
  ...
  useEffect(()=>{void loadAssignments().then(setRows)},[]);
  useEffect(()=>{if(initialHotel)setSelected(initialHotel)},[initialHotel]);
  const aMap=useMemo(()=>assignmentMap(rows),[rows]);
  useEffect(()=>{if(!selected)return;const list=aMap.get(selected.id)||[];setDraft({city_manager:list.find(x=>x.assignment_role==='city_manager')?.user_id||'',account_manager:list.find(x=>x.assignment_role==='account_manager')?.user_id||'',rate_expert:list.find(x=>x.assignment_role==='rate_expert')?.user_id||'',capacity_expert:list.find(x=>x.assignment_role==='capacity_expert')?.user_id||''})},[selected,rows]);
  ...
  async function save(){
    if(!selected)return;
    setBusy(true);setMsg('');
    const db=getSupabase();const next=[...rows];
    try{
      for(const role of Object.keys(ROLE_META) as AssignmentRole[]){
        ...
        if(old&&(!user||old.user_id!==user.id)){
          ...
          if(db){const {error}=await db.from('ihos_hotel_assignments').upsert(closed,{onConflict:'id'});if(error)throw error}
          old=undefined;
        }
        if(user){
...
export function CommunicationsCenterV18({me,hotels=[],users=[],onCreateTask}:any){
  ...
  async function load(){try{const data=await loadAll('ihos_hotel_communications','*');setRows(data.sort((a,b)=>String(b.created_at).localeCompare(String(a.created_at))))}catch{setRows(readLocal<any[]>('ihos-v18-communications',[]))}}
  useEffect(()=>{void load()},[]);
  ...
  async function save(){
    ...
    try{const db=getSupabase();if(db){const {error}=await db.from('ihos_hotel_communications').upsert(row,{onConflict:'id'});if(error)throw error;onlineSaved=true}}catch(e:any){setMsg(`ارتباط محلی ذخیره شد؛ اتصال دیتابیس: ${e.message}`)}
    const next=[row,...rows];setRows(next);writeLocal('ihos-v18-communications',next);
    ...
    if(onlineSaved)setMsg('ارتباط در Timeline هتل ثبت شد');setBusy(false);
  }
...
export function AutomationOperationsV18({automations=[],tasks=[],hotels=[],users=[],onBulkCreate}:any){
  ...
  useEffect(()=>{void loadAssignments().then(setAssignments);void loadAll('ihos_automation_runs','*').then(x=>setRuns(x.sort((a,b)=>String(b.created_at).localeCompare(String(a.created_at))).slice(0,50))).catch(()=>setRuns(readLocal<any[]>('ihos-v18-automation-runs',[])))},[]);
  ...
  async function run(rule:any){
    ...
    try{
      ...
      try{const db=getSupabase();if(db){const {error}=await db.from('ihos_automation_runs').upsert(log,{onConflict:'id'});if(error)throw error}}catch{}
      const next=[log,...runs].slice(0,50);setRuns(next);writeLocal('ihos-v18-automation-runs',next);setPreview({...preview,[rule.id]:found.length});setMsg(`قانون اجرا شد: ${fa(found.length)} تطبیق و ${fa(created.length)} تسک جدید`);
