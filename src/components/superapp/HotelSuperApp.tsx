...
export default function HotelSuperApp({isSuperAdmin=false,actor,onCreateTask,initialTab='overview'}:{isSuperAdmin?:boolean;actor?:{id?:string;username?:string;name?:string};onCreateTask?:(row:Row)=>void;initialTab?:string}){
  ...
  async function importExperts(file:File){
    ...
    try{
      const data=await rowsFromFile(file),appUsers=await loadAll('ihos_users','id,full_name,is_active');const hotelByCode=new Map(master.map(h=>[norm(h.hotel_code),h]));const hotelByTitle=new Map(master.map(h=>[norm(h.title),h]));const merged=new Map<string,any>(),assignmentRows=new Map<string,any>(),missingUsers=new Set<string>();
      const cleanPerson=(value:any)=>norm(value).replace(/خانم|آقای|جناب|سرکار|محترم|دکتر/g,'').replace(/[،,.()]/g,' ').replace(/\s+/g,' ').trim();
      const findUser=(name:string)=>{const needle=cleanPerson(name);return appUsers.find((user:any)=>{const candidate=cleanPerson(user.full_name);return user.is_active!==false&&(candidate===needle||candidate.includes(needle)||needle.includes(candidate))})};
      data.forEach(r=>{const code=text(r,['کد هتل','hotel_code','HotelCode']);const title=text(r,['نام هتل','هتل','hotel','HotelName']);const h=hotelByCode.get(norm(code))||hotelByTitle.get(norm(title));if(!h)return;const taskId=Number(val(r,['taskId','task id','task_id','TaskId','نوع تسک']));const expert=text(r,['First name','نام','نام کارشناس','کارشناس','expert_name','full_name']);if(!expert||![1,2].includes(taskId))return;const row=merged.get(h.id)||{id:`automation-${h.id}`,hotel_id:h.id,hotel_code:h.hotel_code,provider:h.provider||'IHO Provider',hotel_rate:!!h.hotel_rate,hotel_capacity:!!h.hotel_capacity};if(taskId===1)row.capacity_expert=expert;if(taskId===2)row.rate_expert=expert;row.updated_at=new Date().toISOString();merged.set(h.id,row);const user=findUser(expert),role=taskId===1?'capacity_expert':'rate_expert';if(!user){missingUsers.add(expert);return}assignmentRows.set(`${h.id}-${role}`,{id:`assignment-import-${h.id}-${role}`,hotel_id:h.id,hotel_title:h.title,user_id:user.id,user_name:user.full_name,assignment_role:role,is_primary:true,active:true,started_at:new Date().toISOString().slice(0,10),ended_at:null,created_at:new Date().toISOString(),updated_at:new Date().toISOString()})});
      if(!merged.size)throw new Error('هیچ ردیفی تطبیق نشد؛ ستون‌های فایل باید نام هتل، taskId و First name باشند');
      await upsertChunks('ihos_hotel_automation',[...merged.values()],300);
      const assignments=[...assignmentRows.values()],db=getSupabase();let ownershipWarning='';if(assignments.length&&db){try{for(const role of ['rate_expert','capacity_expert']){const ids=[...new Set(assignments.filter(a=>a.assignment_role===role).map(a=>a.hotel_id))];for(let i=0;i<ids.length;i+=250){const{error}=await db.from('ihos_hotel_assignments').update({active:false,ended_at:new Date().toISOString().slice(0,10),updated_at:new Date().toISOString()}).in('hotel_id',ids.slice(i,i+250)).eq('assignment_role',role).eq('active',true);if(error)throw error}}await upsertChunks('ihos_hotel_assignments',assignments,250)}catch{ownershipWarning=' · تاریخچه مالکیت پس از اجرای migration نسخه ۲۲ فعال می‌شود'}}
      sessionStorage.removeItem('ihos-superapp-snapshot-v22');window.dispatchEvent(new CustomEvent('ihos-assignments-updated'));setMsg(`✅ تخصیص ${fa(merged.size)} هتل بروزرسانی و با Hotel Ownership همگام شد${missingUsers.size?` · ${fa(missingUsers.size)} کارشناس در کاربران پیدا نشد`:''}${ownershipWarning}`);await loadData(true)
    }catch(e:any){setMsg(`❌ ${e.message}`)}finally{setBusy(false)}
  }
